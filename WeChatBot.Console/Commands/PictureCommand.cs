    ...
    public static Task<string?> HandlePictureCommand(string _)
    {
        try
        {
            ...
            if (!Directory.Exists(imageFolder))
            {
                ...
                return Task.FromResult<string?>("图片文件夹不存在，请检查配置。");
            }
            ...
            if (!clipboardSuccess)
            {
                return Task.FromResult<string?>("复制图片到剪贴板失败。");
            }

            System.Console.WriteLine($"图片已复制到剪贴板: {imagePath}");

            // 返回特殊标记，表示需要执行粘贴操作
            return Task.FromResult<string?>("##PICTURE##");
        }
        catch (Exception ex)
        {
            System.Console.WriteLine($"处理图片命令时出错: {ex.Message}");
            return Task.FromResult<string?>("处理图片命令时出错，请稍后再试。");
        }
    }
}
